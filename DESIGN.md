...
- **The Hub preview responds to its own box, never to the window.** It is a container (`container-type: inline-size`) and every rule inside it — every `cqw`, every breakpoint for the desktop contents rail — is a `@container` query. The frame switches device by re-rendering the preview at a real device width and scaling the result, so a viewport query would report the studio window and show the desktop layout on the phone setting. **A responsive rule inside a resizable preview is a container query by definition.** The trap: **an element is not its own container.** A `cqw` written on the container resolves against the *next container up* — the viewport, if there is none — and an `@container` block never matches the container itself. So the page gutter lives on a wrapper inside `.hubprev`, not on `.hubprev`, which is also what lets it widen at desktop.
